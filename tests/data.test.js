...
assert.equal(ingredients.versione,5);assert.equal(recipes.versione,8);
for(const r of recipes.ricette){assert(r.nome);assert(Array.isArray(r.ingredienti));for(const row of r.ingredienti)assert(names.has(row[0]),`${r.nome}: ingrediente mancante ${row[0]}`);if(!['colazione','spuntino'].includes(r.tipoPortata))assert.equal(typeof r.richiedeCottura,'boolean',`${r.nome}: richiedeCottura`);assert.equal(typeof r.soloManuale,'boolean');}

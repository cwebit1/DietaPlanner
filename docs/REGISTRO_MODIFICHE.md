...
# Filone: disponibilità delle ricette concrete e gestore visuale

## 1. Disattivazione senza cancellazione — 11 settembre 2026

**Richiesta di Cwe:** usare il futuro file che collega ricette, fotografie e
testi anche come gestore, permettendo di impostare ogni ricetta come disponibile
o non disponibile senza cancellarla.

**Causa e comportamento atteso:** il catalogo visuale definito in precedenza
non possedeva ancora uno stato gestionale e non era implementato. La
disattivazione deve agire sulla singola ricetta concreta identificata da
`nr_<recipeModelId>_<comboIndex>`, non sul nome e non sull'intero template. Una
ricetta disattivata deve restare risolvibile per dettagli, storico e pasti già
salvati, ma non deve poter rientrare in generazione, rigenerazione, Roll o
Salvafrigo.

**Correzione applicata:** creato `db-visuale.json` v1 con 420 record, uno per
ogni ricetta concreta. Ogni record contiene esclusivamente `idRicetta`,
`percorsoImmagine`, `ricettaTestuale` e il booleano `disponibile`, inizialmente
`true`. `motor-v12.js` legge il catalogo, sincronizza nella cache IndexedDB
soltanto il flag gestionale e lo applica in `ricettaAmmessa`; foto e testo non
entrano nel motore o nella cache funzionale. `disponibile:false` è un'esclusione
rigida. Un ID visuale mancante resta invece disponibile, così l'assenza di
contenuti visuali non blocca l'app. `ricetteConVariantIds` applica lo stesso
filtro per Salvafrigo e congelati. La mappa completa per ID viene conservata,
quindi gli snapshot preesistenti continuano a essere materializzabili.

**Gestore:** aggiunto `gestore-ricette.html`, che ricava i nomi dal motore senza
salvarli nel catalogo visuale, consente ricerca e filtri, modifica percorso,
testo e disponibilità, segnala record incompleti e ID orfani e può salvare
direttamente un JSON aperto nei browser compatibili oppure scaricare
`db-visuale.json`. Aggiunto `tools/sincronizza-db-visuale.js` per riallineare gli
ID dopo modifiche deliberate ai gruppi, preservando i contenuti dei record che
mantengono lo stesso ID.

**File modificati o aggiunti:** `motor-v12.js`, `db-visuale.json`,
`gestore-ricette.html`, `tools/sincronizza-db-visuale.js`,
`tests/catalogo-visuale-disponibilita.test.js`,
`docs/SPECIFICA_FUNZIONALE_CORRENTE.md`, `docs/ARCHITETTURA_MOTOR_V12.md`,
`docs/PIANO_REVISIONE_ROOT_NUOVO_DB.md`, `docs/REQUIREMENTS-MATRIX.md`,
`docs/STATO_LOTTI_E_TEST.md`, `docs/REGISTRO_MODIFICHE.md`.

**Verifiche riuscite:** `node --check motor-v12.js`;
`node --check tools/sincronizza-db-visuale.js`;
`node tests/catalogo-visuale-disponibilita.test.js` (420/420 ID, schema minimo,
persistenza del flag, esclusione dal filtro comune, conservazione lookup e
rifiuto duplicati); `node tests/lotto-h-pwa-source-contract.test.js`;
`node tests/lotto-h-approved-recipe-conversion.test.js`;
`node tests/lotto-carboidrati-priorita-pxcuser.test.js` (100/100); parsing di
tutti gli script inline del gestore; parsing JSON; `git diff --check`.

**Esito non nascosto:** `node tests/lotto-g-weekly-generation.test.js`, eseguito
una sola volta, è fallito nel noto scenario instabile della verdura ricorrente
obbligatoria per sette pranzi (`2026-09-03 pranzo`), già documentato prima di
questo intervento. Non è stato rilanciato e non è stata modificata la logica
verdure. La prova visuale del gestore non è stata eseguita perché il browser
remoto ha bloccato l'URL locale con `ERR_BLOCKED_BY_CLIENT`; sintassi e DOM sono
stati controllati staticamente, senza dichiarare una prova browser riuscita.

**SHA dell'intervento tecnico:** `ba309cf`.

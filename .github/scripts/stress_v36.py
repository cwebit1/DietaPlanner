...
require('maxPasti=5' in idx.replace(' ',''),'Limite verdura ricorrente 5 non trovato')
require('maxEsclusioni=7' in idx.replace(' ',''),'Limite esclusioni verdure 7 non trovato')
...
# Proteine: enumera tutti i profili della UI e verifica presenza del blocco di fattibilità.
...
def fattibile(counts, speciali=0):
    free=14-speciali-sum(counts.values())
...
require(invalid>0,'Il test non rileva profili proteici incompatibili')
require('slotLiberi' in idx and ('deficitMinimi' in idx or 'deficit' in idx),'Controllo fattibilità globale proteine non trovato nella UI')
notes.append(f'profili proteici UI esplorati={profiles}; incompatibili da bloccare={invalid}')

# Carboidrati: 100k configurazioni casuali valide.
...
for _ in range(100000):
    ...
    for _j in range(n):
        choices=list(normal)+[k for k,cap in limited.items() if q[k]<cap and limtot<3]
...
notes.append('100000 configurazioni carboidrati simulate senza violazioni dei tetti')

# Poco tempo
require("let pool=['pane','friselle']" in v9_2,'Poco tempo non forza pool pane/friselle')
require('tettoSettimanale||2' in v9_2,'Poco tempo non applica tetto individuale')

# Copertura ricette uniche: completa oppure completabile col jolly pane.
macro_map={'carne_bianca':'carne','carne_rossa':'carne','affettati':'carne','pesce_bianco':'pesce','pesce_azzurro':'pesce','pesce_grande':'pesce','crostacei':'pesce','molluschi':'pesce','pesce_conservato':'pesce','formaggio_fresco':'formaggi','formaggio_stagionato':'formaggi','uova':'uova','legumi':'legumi'}
...
for mac,d in coverage.items():require(d['complete']+d['bread_fix']>0,f'Nessuna copertura unico per macro {mac}')
notes.append('copertura unico per macro: '+json.dumps(coverage,ensure_ascii=False))

# Verdure
require('verduraRicorrenteVariantId' in v10,'Verdura ricorrente non collegata al v10')
require('verdureDisattivate' in v10,'Esclusioni verdure non collegate al v10')
require('ordineVerdureGiornoV17' in v10 and 'ordineVerdureGiornoV17' in idx,'Priorità temporale verdure non collegata')
...
report=['# Stress test motore v36','',f'Esito: **{"OK" if not fail else "FAIL"}**','', '## Controlli']+[f'- {x}' for x in notes]
if fail:report+=['','## Errori']+[f'- {x}' for x in fail]
Path('STRESS-V36-REPORT.md').write_text('\n'.join(report)+'\n',encoding='utf-8')
print('\n'.join(report))
if fail:raise SystemExit(1)
# trigger v36 workflow retry

...
# Collegamenti reali della correzione.
...
# Modello astratto della regola v37: massimizza i jolly rapidi senza perdere quote.
def sim(slots,quick_slots,quota):
    R=quota;S=slots;jolly=0;quota_usata=0
    for i in range(slots):
        quick=i in quick_slots
        if quick and (R==0 or S>R):
            jolly+=1
        elif R>0:
            R-=1;quota_usata+=1
        # se non quick e nessuna quota, lo slot verrà completato dalla rotazione ordinaria
        S-=1
...
for slots in range(0,15):
    ...
    # tutte le cardinalità quick; campiona insiemi quando combinatoria cresce
...
# Stress casuale su 14 slot, includendo 0-2 speciali che riducono la capacità ordinaria.
for _ in range(100000):
    speciali=random.randint(0,2)
    slots=14-speciali
...
# Tetto friselle: la selezione contabile deve avere cap 1 e la ricetta visualizzata segue la stessa famiglia.
